...
-- and no month is missing. Spark SQL dialect (Fabric Spark). Same assertion as
...
-- to read two tables.
--
-- Spark's write here is insert-only (merge with skip_matched_step) on a genuinely unique key, so a
-- row already present cannot be inserted again by a well-behaved run; what this catches on this
-- engine is a Livy session that died mid-file, and a month that landed but was never folded in.
