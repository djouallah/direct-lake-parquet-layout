-- catalog_sales holds exactly the rows download_tpcds.py logged for it, and no fewer.
--
...
-- free of.
--
-- Spark SQL (Fabric Spark) dialect. The other two dialect folders hold the same assertion in their own SQL and
...
WITH stored AS (
  SELECT COUNT(*) AS n FROM {{ ref('catalog_sales') }}
),
landed AS (
  SELECT SUM(row_count) AS n
  FROM {{ ref('stg_tpcds_archive_log') }}
  WHERE source_type = 'catalog_sales'
)
SELECT
