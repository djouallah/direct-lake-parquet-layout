...
# --------------------------------------------------------------- the OneLake upload retry
#
# NOT ABOUT SQL, and here anyway, because this file is where download_tpcds.py's behaviour is
# covered offline and a second file for four tests would be the drift this repo is built against.
# None of these touches the `con` fixture, so they run even where the tpcds extension is missing.
#
# Run 33742041035 is what they are for: `land` died on ten OneLake 500s partway through uploading
# SF100 -- after dsdgen, after the customisation, after the whole scale factor was on local disk.


class _Boom(Exception):
    """An exception carrying a `response.status_code`, the shape `requests` raises."""
...
def test_a_transient_is_retried_and_the_result_is_returned(monkeypatch):
    """The whole point: a 500 costs a wait, not the run."""
    ...
    assert G._retry_onelake("upload store_sales", flaky) == "landed"
    assert len(calls) == 3


def test_a_dropped_connection_is_retried_too(monkeypatch):
    """A transient does not always arrive as an HTTP status -- a reset socket or a read timeout is
    the same OneLake hiccup wearing a different exception, and refusing to retry it would leave the
    commonest failure uncovered."""
    monkeypatch.setattr(G.time, "sleep", lambda _s: None)
    calls = []
    ...
    assert G._retry_onelake("upload catalog_sales", flaky) == "landed"
    assert len(calls) == 2


def test_a_4xx_is_refused_immediately(monkeypatch):
    """A bad path, an expired token or a missing permission is a REFUSAL. Retrying it spends the
    budget and fails anyway, five minutes later, with the cause five screens further up."""
    monkeypatch.setattr(G.time, "sleep", lambda _s: None)
    calls = []
    ...
    with pytest.raises(_Boom):
        G._retry_onelake("upload item", denied)
    assert calls == [1], "a 4xx must cost exactly one attempt"


def test_it_gives_up_and_raises_rather_than_looping(monkeypatch):
    """The budget is finite and the LAST failure is what propagates, so the log names the real
    ...
    with pytest.raises(_Boom):
        G._retry_onelake("upload date_dim", always)
    assert len(calls) == G._UPLOAD_ATTEMPTS


def test_the_retry_names_what_it_was_uploading(monkeypatch, capsys):
    """A partial SF100 upload has to say WHERE it stopped; counting files in OneLake afterwards is
    not a diagnostic anyone should have to do."""
    monkeypatch.setattr(G.time, "sleep", lambda _s: None)
    calls = []
