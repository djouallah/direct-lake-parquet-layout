...
{#-- The wide raw fact — this dataset's MART, the table under layout test. See the duckdb copy's
     ...
     No month_key, no partitioning, no surrogate key. A stored column on 1.5B rows is the month_key
...
{%- set new_files = new_parquet_files('yellow', this if is_incremental() else none) -%}
