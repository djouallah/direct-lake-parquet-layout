-- The 265 TLC taxi zones, from the lookup download_green_taxi.py lands under parquet_raw/zone/.
-- Same lookup file the yellow dataset uses — the zones are fleet-independent — landed into this
-- dataset's own lakehouse by its own downloader. Small and slowly-changing, so it is rebuilt whole
-- rather than merged -- delete+insert on the key matches how dim_green_date is written.
--
