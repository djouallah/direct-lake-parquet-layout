...
-- One-off, fixed date dimension over the green-taxi archive's span. Built in full on the first
...
-- `date` is reserved. The span stays 2011..2026 even though the archive starts 2014 — see the
-- duckdb copy's header.
--
...
-- by cross-joining digit tables and offset from the start date. Identical construction to the AEMO
-- dim_calendar; only the span and the extra day_of_week column differ.
WITH digits(n) AS (
...
calendar AS (
  SELECT DATEADD(DAY, num, CAST('2011-01-01' AS DATE)) AS d
  FROM numbers
  WHERE num <= DATEDIFF(DAY, CAST('2011-01-01' AS DATE), CAST('2026-12-31' AS DATE))
)
SELECT
