...
--
-- The archive is homogeneous by construction — download_green_taxi.py rewrites every month to the
-- canonical 20-column schema before uploading. That normalisation exists FOR THIS LEG: Spark
-- refuses a parquet scan whose files disagree on a column's type ("Failed to merge incompatible
-- data types"), with or without mergeSchema, and TLC's own green files disagree even more than
-- yellow's — the same column ships as INT64, INT32 and DOUBLE in different years. The CASTs below
-- are therefore no-ops over already-canonical data, kept as the explicit declaration that all four
-- engines store the same types.
{%- set cols = green_trip_columns() -%}
{%- set green_root = get_parquet_archive_path() ~ '/green' -%}
...
-- depends_on: {{ ref('stg_green_archive_log') }}

{% set new_files = spark_new_parquet_files('green', this, log_model='stg_green_archive_log') if is_incremental() else [] %}
...
SELECT
  {%- for name in cols %}
  CAST(t.{{ name }} AS {{ green_trip_type(name, 'fabricspark') }}) AS {{ name }},
  {%- endfor %}
  CAST(t.lpep_pickup_datetime AS DATE) AS pickup_date,
  {{ parse_filename('t._metadata.file_name') }} AS file
FROM parquet.`{{ green_root }}{% if is_incremental() %}/{{ '{' ~ new_files | join(',') ~ '}' }}{% endif %}` AS t
{% endif %}
