-- Date dimension over the on-time archive's span. Spark builds the range with
...
SELECT
  ...
  -- (The FACT's DayOfWeek is different on purpose: it is BTS's own published 1=Monday..7=Sunday
  -- column, stored verbatim as source data on every engine alike.)
  CAST(dayofweek(d) - 1 AS INT) AS day_of_week
FROM (
  SELECT explode(sequence(to_date('1987-01-01'), to_date('2026-12-31'), interval 1 day)) AS d
)
{% if is_incremental() %}
