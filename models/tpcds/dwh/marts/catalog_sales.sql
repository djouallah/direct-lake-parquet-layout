-- TPC-DS `catalog_sales` -- the second fact table of the white paper's subset.
--
-- A PASS-THROUGH. The white paper's section 4.5 customisation happens at LAND time, in
-- download_tpcds.py, so the parquet under parquet_raw/catalog_sales/ already IS the paper's table: the facts
...
-- Not a merge: with no `file` column and no pending-file list there is nothing to merge ON,
-- and the whole table is rewritten every dispatch in any case.
{%- set cols = tpcds_columns('catalog_sales') -%}
...
{%- endfor %}
FROM {{ openrowset_parquet(get_parquet_archive_path() ~ '/catalog_sales/*.parquet') }} AS src
