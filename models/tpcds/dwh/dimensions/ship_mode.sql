-- TPC-DS `ship_mode` -- the ship-mode dimension.
--
-- A PASS-THROUGH. The white paper's section 4.5 customisation happens at LAND time, in
-- download_tpcds.py, so the parquet under parquet_raw/ship_mode/ already IS the paper's table: the facts
...
{%- set cols = tpcds_columns('ship_mode') -%}
...
{%- endfor %}
FROM {{ openrowset_parquet(get_parquet_archive_path() ~ '/ship_mode/*.parquet') }} AS src
