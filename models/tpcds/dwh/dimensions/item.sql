-- TPC-DS `item` -- the item dimension.
--
-- A PASS-THROUGH. The white paper's section 4.5 customisation happens at LAND time, in
-- download_tpcds.py, so the parquet under parquet_raw/item/ already IS the paper's table: the facts
...
{%- endfor %}
FROM {{ openrowset_parquet(get_parquet_archive_path() ~ '/item/*.parquet') }} AS src
