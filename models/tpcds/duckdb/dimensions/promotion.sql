-- TPC-DS `promotion` -- the promotion dimension.
--
-- A PASS-THROUGH. The white paper's section 4.5 customisation happens at LAND time, in
-- download_tpcds.py, so the parquet under parquet_raw/promotion/ already IS the paper's table: the facts
...
{%- set cols = tpcds_columns('promotion') -%}
--
...
{%- endfor %}
FROM read_parquet('{{ get_parquet_archive_path() }}/promotion/*.parquet')
