-- Date dimension over the Open Payments archive's span. Spark builds the range with
...
-- The span stays 2013-01-01..2027-12-31 even though the catalog starts at PY2019 -- see the duckdb
-- copy's header for why narrowing it would inner-join out-of-year payment dates away in DAX.
{{ config(
...
FROM (
  SELECT explode(sequence(to_date('2013-01-01'), to_date('2027-12-31'), interval 1 day)) AS d
)
{% if is_incremental() %}
