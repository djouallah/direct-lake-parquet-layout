...
{#-- `auto` is duckrun's own picker: it profiles the data and chooses the key, and it is the
     dispatch default now. It must be passed as a SCALAR — duckrun raises on `['auto']`, because a
     list means "these columns are the key". Any other value is a comma-separated column list, and
     blank means no sort at all.
