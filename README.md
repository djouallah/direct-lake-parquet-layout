...
Medians per run, each dataset filtered to its largest source generation. `build` is compute + storage; `V-Order` is measured off the parquet, not the profile name; `dict encoding` is yes when no mart column fell back to PLAIN data pages (dictionary overflow — what makes a segment expensive to transcode); `avg row group` is the mart's rows per row group — the segment size Direct Lake transcodes — and `table size` sums every table the leg wrote, which is what the storage column is billed for.
