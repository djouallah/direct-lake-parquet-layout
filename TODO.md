...
## 4 of 16 layout groups are EXCLUDED from *Cost and speed by parquet layout*
...
Twelve rows survive today, all complete.
...
No group is in that state today — all 16 carry analytics CU, so the 4 that are missing are missing a
...
Four groups have never been built at 8 vCores. All four are duckrun; every one exists only at 64
...
and a band of the row-group count, so every one of the four below is a layout the nightly never
...
**Three of the original seven have been dispatched by hand and have landed** — they are on the page
...
**The cost is real and is the reason this is a to-do rather than a task.** Four from-scratch builds
of 370M rows plus their query passes. At 8 vCores the CU rate is `cores / 2` = 4/s and a build reads
~10,000 CU, so each is ~40 minutes of compute and the set is roughly **40,000 CU**.
...
for spec in "date,time,DUID:6000000" "date,DUID,time:6000000"             "date,time,price:6000000" "date,time:1000000"; do
